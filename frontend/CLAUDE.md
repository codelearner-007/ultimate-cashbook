...
- **FAB / list bottom-offset fix (`BooksView.jsx`):** `fabBottom` and `listPaddingBottom` are props tuned against `bottomNav`'s (and, for `AdminBooksScreen`, the dashboard `AdminTabBar`'s) height *before* those bars added `insets.bottom` to their own padding. Once that safe-area padding was added to the bars, the FAB's fixed `bottom` position no longer moved with it, so on devices with a tall gesture-nav/home-indicator inset the "ADD NEW BOOK" FAB crowded or nearly touched the now-taller bar instead of keeping its original gap above it. Fixed by computing `fabBottomWithInset = fabBottom + navInset` and `listPaddingBottomWithInset = listPaddingBottom + navInset` inside `BooksView` (one `useSafeAreaInsets()` call, already present) and using those instead of the raw props for the FAB's `bottom` style and both the `FlatList`/`DraggableList` bottom content padding — restores the same visual gap the FAB had above the bar regardless of the device's inset size. Applies to both callers: `BooksScreen` (`fabBottom={80}`, `listPaddingBottom={130}`, own `bottomNav`) and `AdminBooksScreen` (`fabBottom={16}`, `listPaddingBottom={96}`, sits above the dashboard `AdminTabBar` instead).
