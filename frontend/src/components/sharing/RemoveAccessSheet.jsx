...
export default function RemoveAccessSheet({
  visible, onDismiss, onConfirm, share, bookName, isLoading, C, Font,
}) {
  const insets    = useSafeAreaInsets();
  const slideY    = useRef(new Animated.Value(500)).current;
  const bgOpacity = useRef(new Animated.Value(0)).current;

  const displayName = share?.shared_with?.full_name || share?.shared_with?.email || 'This person';
      ...
      <View style={s.anchor} pointerEvents="box-none">
        <Animated.View style={[s.sheet, { backgroundColor: C.card, paddingBottom: 36 + insets.bottom, transform: [{ translateY: slideY }] }]}>
          ...
          <View style={s.headerRow}>
            ...
            <View style={{ flex: 1 }}>
              <Text style={[s.title, { color: C.text, fontFamily: Font.bold }]}>Remove Access</Text>
          ...
          <Text style={[s.body, { color: C.textMuted, fontFamily: Font.regular }]}>
            <Text style={{ color: C.text, fontFamily: Font.semiBold }}>{displayName}</Text>
            {' '}will lose access to{' '}
            <Text style={{ color: C.text, fontFamily: Font.semiBold }}>"{bookName}"</Text>
            {'. They will no longer be able to view or edit this book.'}
          </Text>
          ...
          <View style={s.btnRow}>
            ...
            <TouchableOpacity
              style={[s.btn, s.btnRemove, { backgroundColor: C.danger, opacity: isLoading ? 0.7 : 1 }]}
              onPress={onConfirm}
              disabled={isLoading}
              activeOpacity={0.85}
            >
...
const s = StyleSheet.create({
  ...
  body: {
    ...
    marginBottom: 22, paddingHorizontal: 2,
  },
