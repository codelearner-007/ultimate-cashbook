...
export default function ClearLocalDataSheet({ visible, onDismiss, onConfirm, isLoading, stats, C, Font }) {
      ...
      <View style={s.anchor} pointerEvents="box-none">
        <Animated.View style={[s.sheet, { backgroundColor: C.card, transform: [{ translateY: slideY }] }]}>
          ...
          <View style={s.headerRow}>
            ...
            <View style={{ flex: 1 }}>
              <Text style={[s.title, { color: C.text, fontFamily: Font.bold }]}>Clear Local Data</Text>
          ...
          <Text style={[s.body, { color: C.textMuted, fontFamily: Font.regular }]}>
            {`${books} book${books !== 1 ? 's' : ''} and ${entries} entr${entries === 1 ? 'y' : 'ies'} will be deleted from this device. Your cloud data is not affected.`}
          </Text>
