...
/**
 * Two-step destructive confirmation sheet for "Delete Account".
 * Step 1: Warn the user about what will be deleted.
 * Step 2: Final confirm button (red, explicit label).
 *
 * Unlike FreshStartSheet (which only erases book/entry data), this deletes
 * the Supabase Auth user itself — the account cannot be signed back into.
 *
 * Props:
 *   visible    — boolean
 *   onDismiss  — () => void
 *   onConfirm  — () => void
 *   isLoading  — boolean
 *   statusLabel — string, shown next to the spinner while isLoading
        ...
        <Animated.View style={[s.sheet, { backgroundColor: C.card, transform: [{ translateY: slideY }] }]}>
          ...
          {step === 1 ? (
            <>
              ...
              {[
                { icon: 'user',        label: 'Your profile and sign-in — you will not be able to log back in' },
                { icon: 'book',        label: 'Every book and entry you own, in the cloud and on this device' },
                { icon: 'paperclip',   label: 'Receipt attachments and your profile photo' },
                { icon: 'users',       label: 'Any shared-book access you gave or received' },
              ].map((item) => (
            ...
            <>
              ...
              <Text style={[s.body, { color: C.textMuted, fontFamily: Font.regular }]}>
                You are about to permanently delete your account. There is no way to recover it or its data afterward.
              </Text>
              ...
              <View style={[s.finalWarnBox, { backgroundColor: C.dangerLight, borderColor: C.danger + '66' }]}>
                ...
                <Text style={[s.finalWarnSub, { color: C.danger + 'CC', fontFamily: Font.regular }]}>
                  Your account, profile, and all owned books and entries will be erased.
                </Text>
              </View>
              ...
              {isLoading ? (
                ...
                <View style={s.btnRow}>
                  ...
                  >
                    ...
                    <Text style={[s.btnText, { color: '#fff', fontFamily: Font.bold }]}>Delete My Account</Text>
                  </TouchableOpacity>
                </View>
              )}
            </>
