...
export default function SyncConfirmSheet({ visible, onDismiss, onConfirm, isLoading, stats, C, Font }) {
      ...
      <View style={s.anchor} pointerEvents="box-none">
        <Animated.View style={[s.sheet, { backgroundColor: C.card, transform: [{ translateY: slideY }] }]}>
          ...
          <View style={s.headerRow}>
            <View style={[s.iconCircle, { backgroundColor: C.primary, shadowColor: C.primary }]}>
              <Feather name="upload-cloud" size={20} color="#fff" />
            </View>
            <View style={{ flex: 1 }}>
              <Text style={[s.title, { color: C.text, fontFamily: Font.bold }]}>Sync to Cloud</Text>
          ...
          <Text style={[s.body, { color: C.textMuted, fontFamily: Font.regular }]}>
            {`${books} book${books !== 1 ? 's' : ''} and ${entries} entr${entries === 1 ? 'y' : 'ies'} will be uploaded to your cloud account and merged with any existing data.`}
          </Text>
          ...
          <View style={s.btnRow}>
            ...
            <TouchableOpacity
              style={[s.btn, s.btnPrimary, { backgroundColor: isLoading ? C.primaryLight : C.primary }]}
              onPress={onConfirm}
              disabled={isLoading}
              activeOpacity={0.85}
            >
              ...
              <Text style={[s.btnText, { color: isLoading ? C.primary : '#fff', fontFamily: Font.bold }]}>
