    ...
    <View style={[s.entryCard, { backgroundColor: C.card, borderColor: C.border }]}>
      ...
      <View style={s.entryMid}>
        ...
        <Text style={[s.entryMeta, { color: C.textMuted }]}>
          {item.type === 'in' ? 'Cash In' : 'Cash Out'}  ·  {fmt12h(item.entry_time)}
        </Text>
      </View>
    ...
    <SafeAreaView style={[s.safe, { backgroundColor: C.background }]}>
      ...
      <View style={[s.header, { backgroundColor: C.primary }]}>
        ...
        <View style={s.headerCenter}>
          <Text style={[s.headerTitle, { fontFamily: Font.bold }]} numberOfLines={1}>{categoryName}</Text>
          <Text style={[s.headerSub, { fontFamily: Font.regular }]}>Category Balance</Text>
        </View>
        <View style={{ width: 40 }} />
      </View>
      ...
      {isLoading ? (
        ...
        <View style={s.empty}>
          ...
          <Text style={[s.emptySub, { color: C.textMuted, fontFamily: Font.regular }]}>
            Entries assigned to this category will appear here.
          </Text>
        </View>
      ) : filtered.length === 0 ? (
        <View style={s.empty}>
...
const makeStyles = (C, Font) => StyleSheet.create({
  ...
  entryBadge: {
    ...
    marginLeft: 10, marginRight: 8, minWidth: 48,
    alignItems: 'center', justifyContent: 'center',
  },
  entryBadgeText: { fontSize: 10, fontFamily: Font.bold, lineHeight: 15 },
  entryMid:       { flex: 1, marginRight: 4, paddingVertical: 10 },
  ...
  entryAmount:    { fontSize: 13, fontFamily: Font.bold, lineHeight: 19, minWidth: 64, textAlign: 'right', paddingRight: 12 },
