    ...
    return (
      <View>
        ...
        {!isCollapsed && group.items.map((entry) => (
          <EntryCard key={entry.id} item={entry} C={C} Font={Font} s={s} isDark={isDark} />
        ))}
      </View>
    );
    ...
    <SafeAreaView style={[s.safe, { backgroundColor: C.background }]}>
      ...
      <View style={[s.header, { backgroundColor: C.primary }]}>
        ...
        <View style={s.headerCenter}>
          <Text style={[s.headerTitle, { fontFamily: Font.bold }]} numberOfLines={1}>{modeName}</Text>
          <Text style={[s.headerSub, { fontFamily: Font.regular }]}>Balance Details · Payment Mode</Text>
        </View>
        <View style={{ width: 40 }} />
      </View>
      ...
      <View style={[s.summaryWrap, { backgroundColor: C.card, borderBottomColor: C.border }]}>
        <View style={[s.summaryCard, { borderColor: C.border }]}>
          ...
          <SummaryCard label="Net Balance" value={Math.abs(balance).toLocaleString()} color={balance >= 0 ? C.cashIn : C.danger} C={C} Font={Font} />
          <View style={[s.summaryDivider, { backgroundColor: C.border }]} />
      ...
      {isLoading ? (
        ...
        <View style={s.empty}>
          ...
          <Text style={[s.emptySub, { color: C.textMuted, fontFamily: Font.regular }]}>
            Entries using this payment mode will appear here.
          </Text>
        </View>
      ) : filtered.length === 0 ? (
        <View style={s.empty}>
...
const makeStyles = (C, Font) => StyleSheet.create({
  ...
  dateLabel: { flex: 1, fontSize: 11, fontFamily: Font.semiBold, textTransform: 'uppercase', letterSpacing: 0.8, lineHeight: 16 },
  ...
  entryBadge:     { borderRadius: 7, paddingHorizontal: 8, paddingVertical: 4, marginLeft: 10, marginRight: 10, minWidth: 52, alignItems: 'center', justifyContent: 'center' },
  ...
  onlyYouText: { fontSize: 11, fontFamily: Font.regular, color: C.textSubtle, lineHeight: 16 },

  empty:        { flex: 1, alignItems: 'center', justifyContent: 'center', gap: 12, paddingBottom: 60 },
  emptyIconBox: { width: 80, height: 80, borderRadius: 24, backgroundColor: C.primaryLight, alignItems: 'center', justifyContent: 'center' },
