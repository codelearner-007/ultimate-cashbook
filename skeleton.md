...
| Button                              | Action | Result                                                                                                                                                   |
|-------------------------------------|--------|----------------------------------------------------------------------------------------------------------------------------------------------------------|
| **Export as PDF** (red border)      | Tap    | `canExport` is now `true` for every tier (`export_reports: 'free'` in `frontend/src/lib/canAccess.js`) — `api.post('/api/v1/books/:id/report/pdf', body, { responseType: 'blob' })` where `body` carries `book_name`, `currency`, `date_from`/`date_to`, display `filters`, and the already client-filtered `entries` array; blob → base64 → written to cache dir → `Sharing.shareAsync()` opens native share sheet. The backend endpoint is a stateless renderer with no DB lookup and no tier check (any authenticated user can call it), so this was a pure frontend unlock — and it now works for local-only (never cloud-synced) free-tier books too, since it no longer depends on the book existing in the cloud DB. |
| **Export as Excel** (green border)  | Tap    | Same flow but `POST /api/v1/books/:id/report/excel` → `.xlsx` → `Sharing.shareAsync()`; unlocked for every tier the same way as PDF above                 |
