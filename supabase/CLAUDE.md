...
25. `supabase/migrations/027_attachments_public_read.sql` — adds `attachments_public_read` RLS policy (`to public`) on `storage.objects` so React Native `<Image>` can load attachment URLs without an auth header (bucket is public but old policy was `to authenticated` only)
...
- Public URL format: `https://<project>.supabase.co/storage/v1/object/public/attachments/{user_id}/{entry_id}/attachment.{ext}`
- Write (upload/delete) restricted to the owning user via RLS; reads open to public (URLs are UUID-based and unguessable)
- Backend returns `get_public_url()` on upload; `_resolve_attachment_urls()` in `entries.py` regenerates fresh public URLs from `attachment_path` on every `GET /entries`
...
### Storage policies
```sql
-- attachments (public bucket — write restricted to owner, read open to all)
